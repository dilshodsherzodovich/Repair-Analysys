...
/**
 * The original single-page passport (info card → section tabs → editable
 * component list), restored from commit 634c566. This is the default view;
 * `?design=new` switches to the dashboard rewrite in
 * `locomotive-passport-form.tsx`. Uses the `LocomotivePassportForm` translation
 * namespace, which the rewrite left in place.
 */
...
export default function LocomotivePassportClassic({
  depotId,
  locomotiveId,
}: LocomotivePassportClassicProps) {
    ...
    <div className="space-y-6 pb-4 max-w-full overflow-x-hidden">
      <div className="flex justify-between gap-3 items-center">
        ...
        <div className="flex gap-3">
          ...
          {/* Opens the detailed dashboard as its own page — it has no
              navigation of its own, so it is closed rather than exited. */}
          <Button
            onClick={() =>
              window.open(
                `/depo/${depotId}/locomotive/${locomotiveId}?design=new`,
                "_blank"
              )
            }
            variant="outline"
